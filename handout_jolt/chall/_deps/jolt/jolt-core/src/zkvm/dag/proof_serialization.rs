...
impl<F: JoltField, PCS: CommitmentScheme<Field = F>, FS: Transcript> JoltProof<F, PCS, FS> {
    #[cfg(test)]
    pub fn mutate_opening_claims_for_test(&mut self, f: impl FnOnce(&mut Claims<F>)) {
        f(&mut self.opening_claims);
    }
    ...
    pub fn from_prover_state_manager(mut state_manager: StateManager<'_, F, FS, PCS>) -> Self {
        ...
        let mut openings = std::mem::take(&mut prover_state.accumulator.borrow_mut().openings);
        ...
        for (key, (_pt, claim)) in openings.iter_mut() {
            // if *key == target_key { ... }
            // *claim += F::one();
        ...
        */
        ...
        for (key, (_, claim)) in self.opening_claims.0.iter() {
            ...
            if cnt == 0 {
                println!("{}", c);
                let x = F::from_bytes(&[
                    217, 93, 72, 152, 90, 235, 195, 59, 213, 167, 167, 84, 99, 85, 4, 70, 87, 193, 119, 59, 27, 207, 234, 45, 185, 57, 251, 191, 241, 57, 241, 21
                ]);
            ...
            if cnt == 3 {
                println!("{}", c);
                let x = F::from_bytes(&[
                    118, 17, 65, 155, 236, 22, 13, 199, 57, 11, 82, 38, 138, 140, 25, 230, 183, 18, 176, 196, 57, 156, 25, 145, 63, 248, 73, 241, 239, 109, 251, 33
                ]);
            ...
            if cnt == 50 {
                println!("{}", c);
                let x = F::from_bytes(&[
                    34, 3, 158, 196, 28, 39, 114, 255, 12, 5, 104, 82, 65, 225, 195, 164, 118, 78, 135, 126, 151, 189, 25, 179, 208, 251, 52, 3, 52, 59, 184, 45
                ]);
            ...
            if cnt == 72 {
                println!("{}", c);
                let x = F::from_bytes(&[
                    18, 214, 79, 173, 183, 10, 104, 156, 36, 138, 51, 192, 35, 224, 79, 237, 202, 202, 251, 171, 145, 226, 118, 106, 198, 242, 110, 182, 137, 134, 63, 16
                ]);
            ...
            if cnt == 79 {
                println!("{}", c);
                let x = F::from_bytes(&[
                    202, 6, 81, 197, 162, 183, 193, 226, 62, 169, 169, 136, 38, 185, 32, 67, 194, 52, 239, 184, 56, 27, 217, 213, 77, 5, 32, 41, 250, 247, 80, 28
                ]);
            ...
            opening_accumulator
                .openings_mut()
                .insert(*key, (OpeningPoint::default(), c));
        }
